...
public class MedicosEHospitalController implements Initializable{
		...
		delete.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent t) {
				...
				if (listMed.getSelectionModel().isEmpty()) {
					...
					try {
						Main.getEPS().borrarMedicoHospitalE(licencia, n);
					} catch (NotFoundException e) {
						Alert alert = new Alert(AlertType.INFORMATION);
			    		alert.setTitle("Aviso");
			    		alert.setHeaderText(null);
						alert.setContentText("El hospital buscado no existe");
						alert.showAndWait();
					}
				}
	...
	public void search(ActionEvent e) {
		if (nit.getText().isEmpty()) {
			...
			int n = Integer.parseInt(nit.getText());
				...
				try {
					medicos = FXCollections.observableArrayList(Main.getEPS().buscarHospital(n).getMedicosA());
					listMed.setItems(medicos);
					...
					if (medicos.isEmpty()) {
			    		...
			    		alert.setTitle("Aviso");
			    		alert.setHeaderText(null);
						alert.setContentText("El hospital no cuenta con medicos registrados, puede añadir uno en el menu a su izquierda");
						alert.showAndWait();
					}
	...
	public void addMed(ActionEvent e) {
		...
		try {
			Parent root = FXMLLoader.load(getClass().getResource("/application/registrarMedEspHospital.fxml"));
			Scene scene = new Scene(root);
