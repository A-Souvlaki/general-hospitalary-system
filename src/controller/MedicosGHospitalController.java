...
public class MedicosGHospitalController implements Initializable {
		...
		delete.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent t) {
				...
				if (listMed.getSelectionModel().isEmpty()) {
					...
					try {
						Main.getEPS().borrarMedicoHospital(licencia, n);
	...
	public void search(ActionEvent e) {
		if (nit.getText().isEmpty()) {
			...
			int n = Integer.parseInt(nit.getText());
				...
				try {
					medicos = FXCollections.observableArrayList(Main.getEPS().buscarHospital(n).getMedicosB());
					listMed.setItems(medicos);
	...
	public void addMed(ActionEvent e) {
		...
		try {
			Parent root = FXMLLoader.load(getClass().getResource("/application/registrarMedGenHospital.fxml"));
			Scene scene = new Scene(root);
