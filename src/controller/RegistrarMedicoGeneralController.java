...
public class RegistrarMedicoGeneralController implements Initializable {
	...
	@FXML
	private ChoiceBox<String> externo;
	...
	public void addMedical(ActionEvent event) {
		try {
			...
			alert.setTitle("Error");
			alert.setHeaderText(null);
			alert.setContentText("No existe una clinica con el nit ingresado");
			alert.showAndWait();
			Parent root;
			try {
				root = FXMLLoader.load(getClass().getResource("/application/medicosPuesto.fxml"));
				Scene scene = new Scene(root);
				Stage stage = (Stage)((Node) event.getSource()).getScene().getWindow();
				stage.setScene(scene);
				stage.centerOnScreen();
	...
	public void dataMed() throws NumberFormatException, NotFoundException {
		...
		String espe = externo.getValue();
		MedicoGeneral m = new MedicoGeneral(nombre, ide, no, espe);
		Main.getEPS().añadirMedicoPuesto(m, Integer.parseInt(nit.getText()));
	}
